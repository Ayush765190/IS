...
def create_matrix(key,polybius_text, plain_text):
    ...
    print(arr)
    encoding_first_time(arr,polybius_text, plain_text)

# intermediate cipher function 
def encoding_first_time(arr,polybius_text , plain_text ):
    cipher =[]
    ...
    final_encoding(cipher,"402153", arr)
...
def final_encoding(cipher , key2 , key_arr):
    ...
    decoding(my_string, key2, key_arr)

def decoding(cipher_text, key2, key_arr):
...
def main():
    ...
    key = "himolveyu"
    key2 = "402153"
    plain_text = "youseebutyoudonotobserve"
    polybius_text = "ADFGVX"
    create_matrix(key,polybius_text, plain_text)
